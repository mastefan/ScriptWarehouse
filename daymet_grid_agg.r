...
daymet_grid_agg = function(file = NULL,
  ...
  # remove path from file for naming purposes
  file <- basename(file)
  ...
  if (ext == 'tif' | ext == 'nc'){
    if (ext == 'nc'){ # extract dates from .nc file
      ...
      dy <- as.numeric(as.vector(t(as.data.frame(strsplit(names(data),
                                                          "[.]"))[2,])))
      yr <- strsplit(names(data), "_")[[1]][3] # splits based on standard daymet file convention
      yr <- strsplit(yr, "[.]")[[1]][1] # additional split to work with data from calc_rfr
  ...
  if (int == 'seasonal'){
    ...
    year_start <- head(dates, n = 1)
    year_end <- tail(dates, n = 1)
  ...
  if (internal == FALSE){
      ...
      output_file <- sprintf('%s_%s%s_%s_agg.tif', param, int, FUN, yr)
